...
function page() {
  ...
  const onSubmit = async (data) => {
    let preOrders= await fetch("http://localhost:3000/api/user",{
      method: "PUT",
            headers:{
    ...
    for (let i = 0; i < Carts.Carts.length; i++) {
      ...
      let res= fetch("http://localhost:3000/api/order",{
        method: "POST",
        headers:{
          'Content-Type': 'application/json'
        },
    ...
    await fetch("http://localhost:3000/api/user",{
      ...
      body: JSON.stringify({filter:{email:session.user.email}, update: {orders: orderarray}})
    })
    Carts.setCarts([]);
    await fetch("http://localhost:3000/api/user",{
      method: "PATCH",
      ...
      body: JSON.stringify({filter:{email:session.user.email}, update: {carts: []}})
    })
  ...
  return (
    <div className="min-h-[90vh]">
      <div className="2xl:w-[1532px] w-[100%] justify-self-center">
        ...
        >
        {Carts.Carts.length !==0 ? <div className="max-w-[510px]">
          ...
          </div>: <div className="max-w-[510px] flex flex-col items-center justify-center gap-3">
            <div className="text-xl font-medium flex items-center gap-3"><BsCartXFill size={32}/> Your Cart is empty Click here</div>
            <Link href="http://localhost:3000/collection"><div><button className="bg-black cursor-pointer px-5 py-2 rounded-[5px] text-lg text-white">Items</button></div></Link>
          </div>
}
          <div className="">
